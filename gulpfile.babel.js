// Generated on 2017-06-29 using generator-angular-fullstack 4.2.2
"use strict";
...
const mozaikMicroservicePath = "mozaik_microservice";
const paths = {
  client: {
...
gulp.task("start:python-server", (cb) => {
  var spawn = require("child_process").spawn;
  console.info("Starting python");
  var PIPE = { stdio: "inherit" };
  spawn("python3", [`./${mozaikMicroservicePath}/server.py`], PIPE).on(
    "close",
    cb
  );
});
...
gulp.task("serve", (cb) => {
  runSequence(
    ...
    // 'webpack:dev',
    ["start:python-server", "start:server", "start:client"],
    "watch",
    cb
  );
});
...
gulp.task("serve:dist", (cb) => {
  runSequence(
    ...
    "env:all",
    "env:prod",
    ["start:python-server", "start:server:prod", "start:client"],
    cb
  );
});
