  ...
  function ($q) {
    ...
    this.load = function (url) {
      ...
      image.onload = function () {
        ...
        if (
          (typeof this.complete === "boolean" && this.complete === false) ||
...
angular.module("bootstrapLightbox").provider("Lightbox", function () {
  ...
  this.isSharedVideo = function (image) {
    return (
      this.isVideo(image) && !this.getImageUrl(image).match(/\.(mp4|ogg|webm)$/)
    );
  };
  ...
  this.$get = [
    ...
    function ($document, $injector, $uibModal, $timeout, ImageLoader) {
      ...
      Lightbox.calculateImageDimensionLimits =
        this.calculateImageDimensionLimits;
      ...
      Lightbox.openModal = function (newImages, newIndex, modalParams) {
        ...
        Lightbox.modalInstance = $uibModal.open(
          angular.extend(
            {
      ...
      Lightbox.setImage = function (newIndex) {
        ...
        var success = function (properties) {
          ...
          Lightbox.imageCaption =
            properties.imageCaption || Lightbox.getImageCaption(image);
...
angular.module("bootstrapLightbox").directive("lightboxSrc", [
  ...
  function ($window, ImageLoader, Lightbox) {
    ...
    var calculateImageDisplayDimensions = function (
      dimensions,
      fullScreenMode
    ) {
    ...
    return {
      link: function (scope, element, attrs) {
        ...
        var resize = function () {
          ...
          angular
            .element(document.querySelector(".lightbox-modal .modal-dialog"))
            .css({
          ...
          angular
            .element(document.querySelector(".lightbox-modal .modal-content"))
            .css({
        ...
        scope.$watch(
          ...
          function (src) {
            ...
            if (!Lightbox.isVideo(Lightbox.image)) {
              // image
